...
| trust_proxy_addresses                           | TRUST_PROXY_ADDRESSES                             | Empty list []                                                                                                                      | Array of additional trusted proxy IP addresses or CIDR ranges to add to the default ones (loopback, linklocal, uniquelocal). Used by Express `trust proxy` setting for correct client IP detection behind reverse proxies. |
...
| app:app_logs:logs_redacted_inputs | APP__APP_LOGS__LOGS_REDACTED_INPUTS | `["password","secret","token","api_tokens","authentication_value","enterprise_license"]` | List of GraphQL input fields to redact from logs (e.g. `password`) |
...
| app:audit_logs:trace_request_headers | APP__AUDIT_LOGS__TRACE_REQUEST_HEADERS | `["user-agent", "x-forwarded-for"]` | List of HTTP headers to include in audit logs              |
...
| app:concurrency:batch_max_active_executions         | APP__CONCURRENCY__BATCH_MAX_ACTIVE_EXECUTIONS            | 4             | Maximum weighted intact batch GraphQL execution capacity in one backend process; larger batches consume multiple slots based on operation count and encoded payload size |
