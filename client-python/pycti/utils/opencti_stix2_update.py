...
class OpenCTIStix2Update:
    ...
    def add_object_marking_refs(
        ...
        add_marking_definition = self._get_object_marking_ref_adder(entity_type)
        if len(normalized_object_marking_refs) == 1 or any(
            not isinstance(object_marking_ref, str)
        ...
        add_many = self._get_object_marking_ref_add_many(entity_type)
        if add_many is None or not self._supports_bulk_ref_relation_validation():
            for object_marking_ref in normalized_object_marking_refs:
                add_marking_definition(
                    id=entity_id, marking_definition_id=object_marking_ref
                )
    ...
    def _get_object_marking_ref_adder(self, entity_type):
        if entity_type == "relationship":
            return self.opencti.stix_core_relationship.add_marking_definition
        if entity_type == "sighting":
            return self.opencti.stix_sighting_relationship.add_marking_definition
        if StixCyberObservableTypes.has_value(entity_type):
            return self.opencti.stix_cyber_observable.add_marking_definition
        return self.opencti.stix_domain_object.add_marking_definition

    def _get_object_marking_ref_add_many(self, entity_type):
        ...
        if entity_type == "sighting":
            return getattr(
                nested_ref_relationship, "add_many_to_stix_sighting_relationship", None
            )
        return getattr(nested_ref_relationship, "add_many_to_stix_core_object", None)

    def _supports_bulk_ref_relation_validation(self):
        supports_api_feature = getattr(self.opencti, "supports_api_feature", None)
        if supports_api_feature is None:
            return False
        try:
            return supports_api_feature(BULK_REF_RELATION_VALIDATION_API_FEATURE)
        except Exception:
            return False

    def _get_ref_relation_remove_many(self, entity_type, allow_sighting=False):
        nested_ref_relationship = getattr(
            self.opencti, "stix_nested_ref_relationship", None
        )
        if entity_type == "relationship":
    ...
    def _supports_bulk_ref_relation_delete(self):
        supports_api_feature = getattr(self.opencti, "supports_api_feature", None)
        if supports_api_feature is None:
            return False
        try:
            return supports_api_feature(BULK_REF_RELATION_DELETE_API_FEATURE)
        except Exception:
            return False

    def _get_bulk_ref_relation_remover(
        self, entity_type, ref_ids, allow_sighting=False
    ):
        if len(ref_ids) <= 1 or any(not isinstance(ref_id, str) for ref_id in ref_ids):
            return None
        remove_many = self._get_ref_relation_remove_many(
            entity_type, allow_sighting=allow_sighting
        )
        if remove_many is None or not self._supports_bulk_ref_relation_delete():
            return None
        return remove_many
    ...
    def _remove_ref_relations(
        ...
        relationship_type,
        remove_one,
        allow_sighting=False,
        remove_many=None,
    ):
        if len(ref_ids) == 0:
            return
        if remove_many is None:
            remove_many = self._get_bulk_ref_relation_remover(
                entity_type, ref_ids, allow_sighting=allow_sighting
            )
        elif len(ref_ids) <= 1 or any(
            not isinstance(ref_id, str) for ref_id in ref_ids
        ):
            remove_many = None
        if remove_many is None:
    ...
    def _get_object_marking_ref_remover(self, entity_type):
        if entity_type == "relationship":
            return self.opencti.stix_core_relationship.remove_marking_definition
        if entity_type == "sighting":
            return self.opencti.stix_sighting_relationship.remove_marking_definition
        if StixCyberObservableTypes.has_value(entity_type):
            return self.opencti.stix_cyber_observable.remove_marking_definition
        return self.opencti.stix_domain_object.remove_marking_definition
    ...
    def remove_object_marking_refs(
        ...
        remove_marking_definition = self._get_object_marking_ref_remover(entity_type)
    ...
    def add_external_references(
        ...
        for external_reference in external_references:
            ...
            if add_external_reference is None:
                add_external_reference, add_many = (
                    self._get_external_reference_relation_adders(entity_type)
                )
            if add_many is None:
    ...
    def _get_external_reference_relation_adders(self, entity_type):
        nested_ref_relationship = getattr(
            self.opencti, "stix_nested_ref_relationship", None
        )
        if entity_type == "relationship":
