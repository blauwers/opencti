...
def test_copied_context_does_not_share_helper_mutations():
    helper = _helper()

    with helper.request_context():
        helper.work_id = "work-parent"
        copied_context = copy_context()

        def mutate_child_context():
            helper.work_id = "work-child"
            assert helper.work_id == "work-child"
